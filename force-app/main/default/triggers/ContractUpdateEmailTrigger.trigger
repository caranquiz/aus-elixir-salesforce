...
Author: Ariz Mazhary
Description : This trigger fires when Email Related to Contract fires manually. 
...
trigger ContractUpdateEmailTrigger on EmailMessage (after insert) {
    try{
        ...
        if(!(disCustomTrigger.ContractUpdateEmailTrigger__c)){
            if(Trigger.isInsert && Trigger.isAfter){
                ...
                for(collect__Loan_Account__c collectAccount:collectAccountList){
                    ...
                    collectionActivity.collect__Action__c=ConstantValues.EMAIL;
