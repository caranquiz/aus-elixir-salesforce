...
skuid.model.updateData([newAccountModel,newCollateralOwnersModel,newPartyModelClone],function(){
    ...
    if(!newPartyRow.NewRelationship && newPartyModelClone.data && newPartyModelClone.data.length == 1){
        // var partyCloneRow = newPartyModelClone.data[0];
        // var rowUpdates = {};
        // $.each(partyCloneRow,function(fieldId,val) {
        //     if (((fieldId != 'attributes') && (val != null)) || (fieldId != 'Id')) {
        //         var modelField = newPartyModelClone.getField(fieldId);
    ...
    if(newAccountModel.data && newAccountModel.data.length > 0 && newAccountModel.data[0].Business_Information__c){
        newBIModel.setCondition(biIdCondition,newAccountModel.data[0].Business_Information__c);
    }else{
...
function createNewBIRow(newBIModel){
    if(newBIModel.data && newBIModel.data.length < 1){
        var newRow = newBIModel.createRow({
            additionalConditions: [
                //{ field: 'Business_Legal_Name__c', value:'Set Business Info' },
            ]
        });
    }
}
...
if(resObj.status == 'SUCCESS'){
    ...
    window.parent.postMessage({type: 'action-document-and-party-refresh'}, '*');
}else{
